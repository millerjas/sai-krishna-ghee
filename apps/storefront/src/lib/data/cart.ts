...
export async function getOrSetCart(countryCode: string) {
  ...
  if (!cart) {
    ...
    const cartResp = await sdk.store.cart
      .create(body, {}, getAuthHeaders())
      .catch((e) => {
        console.error("Error creating cart in Medusa backend:", e)
        return null
      })
    ...
    if (cartResp?.cart) {
      ...
      cart = cartResp.cart as unknown as B2BCart
    }
  }

  if (cart && region.id && cart?.region_id !== region.id && !region.id.startsWith("reg_default")) {
